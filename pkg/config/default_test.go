...
func TestMachineVolumes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			output:  []string{"/foobar:/foobar", "/foobar1:/foobardest:ro", "/test1:/env1", "/test1:/test1", "/test2:/test1"},
			assert: func(err error, in, out []string, msg string) {
				assert.EqualError(t, err, "invalid machine volume $env3 environment variable is not set", msg)
			},
		},
		{
			...
			output:  []string{"/foobar:/foobar", "/foobar1:/foobardest:ro", "/test1:/env1", "/test1:/test1", "/test2:/test1"},
			assert: func(err error, in, out []string, msg string) {
				assert.EqualError(t, err, "invalid machine volume $env4 environment variable is not set", msg)
			},
		},
		{
			msg:     "This is broken",
			...
			assert: func(err error, in, out []string, msg string) {
				assert.EqualError(t, err, "invalid machine volume /foobar:/foobar:ro:badopt", msg)
			},
		},
		{
			msg:     "This is broken2",
			...
			output:  []string{"/foobar:/foobar:ro"},
			assert: func(err error, in, out []string, msg string) {
				assert.EqualError(t, err, "invalid machine volume /foobar", msg)
			},
		},
	} {
